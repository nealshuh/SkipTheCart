  ...
  try {
    ...
    if (currentSimilarItem) {
      if (similarItems.length > 1) {
        ...
        similarHint.textContent = `Swipe to see ${similarItems.length} wardrobe items`;
      ...
      if (similarItems.length > 1) {
        ...
        prevArrow.addEventListener('click', function() {
          const newIndex = currentSimilarIndex > 0 ? currentSimilarIndex - 1 : similarItems.length - 1;
        ...
        nextArrow.addEventListener('click', function() {
          const newIndex = currentSimilarIndex < similarItems.length - 1 ? currentSimilarIndex + 1 : 0;
          panel.cartState.items[currentIndex].currentSimilarIndex = newIndex;
          processCartItems(cartItems, panel, usedSelector);
        });
      ...
      similarImage.alt = currentSimilarItem.category || 'Wardrobe Item';
      similarImage.onerror = function() {
        this.src = '/api/placeholder/200/250';
      };
      const matchBadge = document.createElement('div');
      matchBadge.className = 'item-match-badge';
      matchBadge.textContent = 'From Wardrobe';
      ...
      similarItemName.textContent = currentSimilarItem.category || 'Wardrobe Item';
      const similarItemLastWorn = document.createElement('div');
      similarItemLastWorn.className = 'item-specs';
      similarItemLastWorn.textContent = `Added on: ${currentSimilarItem.lastWorn}`;
      ...
      noSimilarMessage.textContent = 'No bottoms found in your wardrobe';
    ...
    if (currentSimilarItem) {
      ...
      footerMessage.textContent = `You have ${similarItems.length} bottom(s) in your wardrobe.`;
